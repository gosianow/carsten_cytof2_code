...
plot_heatmaps_for_sign_expr <- function(expr_all, md, FDR_cutoff, pval_name_list, adjpval_name_list, results2plot, breaks, legend_breaks, color_groups, outdir, prefix, prefix2, suffix){
  ...
  if(sum(which_top_pvs) > 0){
    ...
    pheatmap(pvs_heat, cellwidth = 60, cellheight = 24, color = c("grey50", "grey70", "grey90"), breaks = c(0, 0.05, 0.1, 1), legend_breaks = c(0, 0.05, 0.1, 1), legend = FALSE, cluster_cols = FALSE, cluster_rows = FALSE, labels_col = labels_col, labels_row = labels_row, gaps_col = NULL, gaps_row = gaps_row, display_numbers = TRUE, number_format = "%.02e", number_color = "black", fontsize_row = 14, fontsize_col = 14, fontsize = 12, filename = file.path(outdir, paste0(prefix, "expr_", prefix2, "pheatmap0pvs", suffix, ".pdf")))
  ...
  for(i in 1:length(results2plot)){
    ...
    if(sum(which_top_pvs) > 0) {
      ...
      pheatmap(expr, cellwidth = 28, cellheight = 24, color = colorRampPalette(c("#87CEFA", "#56B4E9", "#0072B2", "#000000", "#D55E00", "#E69F00", "#FFD700"), space = "Lab")(100), breaks = breaks, legend_breaks = legend_breaks, cluster_cols = FALSE, cluster_rows = FALSE, labels_col = labels_col, labels_row = labels_row, gaps_col = gaps_col, gaps_row = gaps_row, fontsize_row = 14, fontsize_col = 14, fontsize = 12, annotation_col = annotation_col, annotation_colors = annotation_colors, annotation_legend = FALSE, filename = file.path(outdir, paste0(prefix, "expr_", prefix2, "pheatmap", i, "_", results_names[i], suffix, ".pdf")))
      
      
    }else{
      
      pdf(file.path(outdir, paste0(prefix, "expr_", prefix2, "pheatmap", i, "_", results_names[i],  suffix, ".pdf")))
